...
function isAdmin(req){
  ...
  return raw.split(";").map(s=>s.trim()).includes("adm=1");
}
...
.brand{font-weight:900;font-size:26px;background:linear-gradient(90deg,#111,#334155,#64748b);-webkit-background-clip:text;background-clip:text;color:transparent}
.nav{display:flex;gap:8px;flex-wrap:wrap}
.nav a{padding:8px 12px;border-radius:999px;background:var(--glass);border:1px solid var(--glass-brd);backdrop-filter:blur(8px);text-decoration:none;color:var(--ink);font-weight:600}

/* 메트릭 */
.metrics{display:grid;grid-template-columns:repeat(auto-fit,minmax(160px,1fr));gap:12px;margin:16px 0}
.metric{background:var(--glass);border:1px solid var(--glass-brd);border-radius:16px;padding:14px;backdrop-filter:blur(10px);box-shadow:0 10px 40px rgba(0,0,0,.08)}
.metric .lbl{font-size:12px;color:var(--muted)}
.metric .num{font-size:28px;font-weight:900}

/* 패널/툴바 */
.panel{background:var(--glass);border:1px solid var(--glass-brd);border-radius:18px;padding:16px;backdrop-filter:blur(12px);box-shadow:0 12px 44px rgba(0,0,0,.10)}
.toolbar{display:grid;grid-template-columns:1fr auto;gap:12px;align-items:center;margin-bottom:14px}
.input{display:flex;align-items:center;gap:8px;background:#ffffffc7;border:1px solid #e2e8f0;border-radius:12px;padding:10px 12px}
.input input{border:0;outline:none;background:transparent;width:100%;font-size:15px}
.segment{display:inline-flex;gap:6px;padding:4px;background:#fff;border:1px solid #e2e8f0;border-radius:14px}
.segment button{padding:9px 14px;border:0;background:transparent;cursor:pointer;font-weight:700;color:#64748b;border-radius:10px}
.segment button.active{background:#111;color:#fff}

/* 테이블 껍데기 */
.tableWrap{background:var(--glass);border:1px solid var(--glass-brd);border-radius:24px;padding:8px;backdrop-filter:blur(12px);box-shadow:0 14px 48px rgba(0,0,0,.12)}

/* 표: 칼럼 정렬(정확히 맞춤) */
table{width:100%;border-collapse:separate;border-spacing:0;border-radius:16px;overflow:hidden;table-layout:fixed}
th,td{padding:14px 12px;border-bottom:1px solid #e2e8f0;background:#ffffffcc;text-align:center}
th{background:#f8fafc;font-weight:800}
tr:last-child td{border-bottom:none}
tbody tr:hover td{background:#fff}
th:nth-child(2), td:nth-child(2){text-align:left}  /* 이름만 좌측 */
...
.tok span{padding:6px 12px;border-radius:999px;background:#eef2ff;border:1px solid #dbeafe;color:#1e3a8a;font-weight:700}
.bal{font-weight:900;font-variant-numeric:tabular-nums}
...
<div class="wrap">
  <div class="header">
    ...
    <div class="nav">
      <a href="/admin/cards">카드 관리</a>
      <a href="/admin/booths">부스 관리</a>
      <a href="/login?ref=/">관리자</a>
      <a href="/booth/login?ref=/">부스 로그인</a>
    </div>
  </div>

  <div class="metrics">
    <div class="metric"><div class="lbl">등록 인원</div><div class="num" id="m-count">${rows.length}</div></div>
    <div class="metric"><div class="lbl">총 잔액 합</div><div class="num" id="m-sum">0</div></div>
    <div class="metric"><div class="lbl">평균 잔액</div><div class="num" id="m-avg">0</div></div>
  </div>

  <div class="panel">
    <div class="toolbar">
      <label class="input"><input id="q" placeholder="이름/라벨 검색 (예: 홍길동)"></label>
      <div class="segment">
        <button class="active" data-sort="idx">기본순</button>
        <button data-sort="name">이름순</button>
        <button data-sort="bal-desc">잔액↓</button>
        <button data-sort="bal-asc">잔액↑</button>
      </div>
    </div>

    <div class="tableWrap">
      <table id="tbl" aria-label="카드 목록">
        <colgroup>
        ...
        <thead>
          <tr><th>#</th><th>이름</th><th>라벨</th><th>잔액</th><th>열기</th></tr>
        </thead>
...
// ====== 관리자: 부스 계정 관리 (화이트 글래스 스타일) ======
app.get("/admin/booths", (req, res) => {
  if (!isAdmin(req)) return res.redirect("/login?ref=" + encodeURIComponent(req.originalUrl));
...
.panel{background:var(--glass);border:1px solid var(--glass-brd);border-radius:18px;padding:16px;backdrop-filter:blur(12px);box-shadow:0 12px 44px rgba(0,0,0,.10);margin-bottom:16px}
...
.inputRow{display:grid;grid-template-columns:1fr 1fr 1fr auto;gap:8px}
input,textarea{width:100%;padding:10px 12px;border:1px solid #e2e8f0;border-radius:12px;background:#ffffffd9}
...
.toolbar{display:grid;grid-template-columns:1fr auto;gap:12px;align-items:center;margin-bottom:14px}
.inputSearch{display:flex;align-items:center;gap:8px;background:#ffffffc7;border:1px solid #e2e8f0;border-radius:12px;padding:10px 12px}
.inputSearch input{border:0;outline:none;background:transparent;width:100%;font-size:15px}

.tableWrap{background:var(--glass);border:1px solid var(--glass-brd);border-radius:24px;padding:8px;backdrop-filter:blur(12px);box-shadow:0 14px 48px rgba(0,0,0,.12)}
table{width:100%;border-collapse:separate;border-spacing:0;border-radius:16px;overflow:hidden}
th,td{padding:14px 12px;border-bottom:1px solid #e2e8f0;background:#ffffffcc}
th{background:#f8fafc;font-weight:800}
tr:last-child td{border-bottom:none}
tbody tr:hover td{background:#fff}
.idx{color:#94a3b8;font-weight:700}
...
<div class="wrap">
  ...
  <!-- 새 부스 추가 -->
  <div class="panel">
    <label class="small">새 부스 추가</label>
    <form method="post" action="/admin/booth/create" class="inputRow">
      <input name="username" placeholder="아이디 (예: booth1)" required>
      <input name="label" placeholder="표시명 (예: 1번 부스)" required>
      <input type="password" name="password" placeholder="비밀번호 (예: 1111)" required>
  ...
  <div class="panel">
    <div class="toolbar">
      <label class="inputSearch"><input id="q" placeholder="아이디/표시명 검색"></label>
      <div style="opacity:.8;color:#64748b">총 ${rows.length}개 부스</div>
    </div>

    <div class="tableWrap">
      <table aria-label="부스 목록">
        <thead><tr><th>#</th><th>아이디</th><th>표시명</th><th>작업</th></tr></thead>
        <tbody id="tb">${rowsHtml}</tbody>
      </table>
    </div>
  </div>
...
// -------- 부스 차감 페이지 --------
// ====== 부스: 카드 차감 화면 (화이트 글래스 스타일) ======
app.get("/b/:token", (req, res) => {
  const token = req.params.token;
  ensureCard.run(token);
  const c = getCard.get(token);      // 카드 정보
  const boothUser = getBooth(req);   // 쿠키에 저장된 부스 아이디(없으면 "")

  // 최근 부스 차감 내역(해당 카드 기준, 부스 txn만 강조)
...
.card{background:var(--glass);border:1px solid var(--glass-brd);border-radius:18px;padding:16px;backdrop-filter:blur(12px);box-shadow:0 12px 44px rgba(0,0,0,.10)}
h2,h3{margin:0 0 10px}
.sub{color:#64748b;font-size:13px}

.badge{
  display:inline-block;padding:6px 12px;border-radius:999px;background:#eef2ff;border:1px solid #dbeafe;color:#1e3a8a;font-weight:700
}
.balance{display:flex;align-items:baseline;gap:10px;margin:8px 0 12px}
.balance .num{font-size:44px;font-weight:900}

.actions{display:flex;flex-wrap:wrap;gap:8px;margin:8px 0 12px}
.btn{padding:10px 14px;border:none;border-radius:10px;background:#111;color:#fff;cursor:pointer;font-weight:700}
.btn.primary{background:var(--accent)}
.btn.danger{background:#ef4444}
.row{display:grid;grid-template-columns:1fr auto;gap:8px}
input,textarea{width:100%;padding:10px 12px;border:1px solid #e2e8f0;border-radius:12px;background:#ffffffd9}

.notice{padding:10px 12px;border-radius:12px;background:#fff7ed;border:1px solid #fed7aa;color:#9a3412;margin-bottom:10px}

.tableWrap{background:var(--glass);border:1px solid var(--glass-brd);border-radius:18px;padding:8px;backdrop-filter:blur(12px);box-shadow:0 14px 48px rgba(0,0,0,.12)}
table{width:100%;border-collapse:separate;border-spacing:0;border-radius:14px;overflow:hidden}
th,td{padding:12px 10px;border-bottom:1px solid #e2e8f0;background:#ffffffcc}
th{background:#f8fafc;font-weight:800}
tr:last-child td{border-bottom:none}
.plus{color:#16a34a;font-weight:900}
.minus{color:#dc2626;font-weight:900}
...
<div class="wrap">
  <div class="header">
    <div class="brand">부스 차감</div>
    <div class="nav">
      <a href="/">대시보드</a>
      <a href="/booth/login?ref=${encodeURIComponent("/b/"+token)}">${boothUser ? "부스 전환/로그인" : "부스 로그인"}</a>
      ${boothUser ? '<span class="mono" style="padding:8px 10px;border-radius:10px;background:#ffffffb8;border:1px solid #e2e8f0">현재부스: '+boothUser+'</span>' : ''}
    </div>
  </div>
  ...
  <div class="grid">
    <!-- 좌: 차감 패널 -->
    <div class="card">
      <h2>${token} <span class="badge">잔액</span></h2>
      <div class="balance"><div class="num" id="bal">${c.balance}</div><div class="sub">point</div></div>

      ${boothUser ? '' : '<div class="notice">부스 로그인이 필요합니다. 오른쪽 상단의 “부스 로그인”을 눌러 로그인하세요.</div>'}

      <div class="actions">
        <button class="btn" onclick="dec(1)" ${boothUser?'':'disabled'}>-1</button>
        <button class="btn" onclick="dec(5)" ${boothUser?'':'disabled'}>-5</button>
        <button class="btn" onclick="dec(10)" ${boothUser?'':'disabled'}>-10</button>
      </div>

      <div class="row">
        <input id="n" type="number" min="1" placeholder="차감할 수량 (예: 3)" ${boothUser?'':'disabled'}>
        <button class="btn primary" onclick="bulk()" ${boothUser?'':'disabled'}>차감</button>
      </div>

      <div class="row" style="margin-top:8px">
        <input id="reason" type="text" placeholder="사유/메모 (선택)" ${boothUser?'':'disabled'}>
        <button class="btn" onclick="saveReason()" disabled style="opacity:.6;cursor:not-allowed">메모 저장</button>
      </div>
      <div class="sub" style="margin-top:8px">부스는 <b>차감만</b> 가능하며, 추가/잔액설정은 관리자 페이지에서 수행하세요.</div>
    </div>
    ...
    <div class="card">
      ...
      <div class="tableWrap" style="margin-top:8px">
        <table>
          <thead><tr><th>시간</th><th>변동</th><th>출처</th><th>부스</th><th>사유</th></tr></thead>
          <tbody id="tx">${txHtml}</tbody>
        </table>
      </div>
    </div>
  </div>
...
(function(){
  async function apply(delta, reason){
    const r = await fetch("/api/booth-apply", {
      method:"POST",
      headers:{"Content-Type":"application/json"},
      body: JSON.stringify({ token: "${token}", delta: delta, reason: reason||"booth" })
    });
  ...
  // -1/-5/-10 버튼
  window.dec = async function(n){
    const bal = await apply(-Math.abs(parseInt(n,10)||1), document.getElementById("reason").value||"");
    if(bal==null) return;
    document.getElementById("bal").textContent = bal;
    location.reload(); // 내역 갱신
  };

  // 커스텀 차감
  window.bulk = async function(){
    const v = parseInt(document.getElementById("n").value||"0",10);
    if(!v || v<1){ alert("1 이상 입력하세요"); return; }
    const reason = document.getElementById("reason").value||"";
    const bal = await apply(-v, reason);
    if(bal==null) return;
  ...
  // (옵션) 메모만 따로 저장하고 싶다면 서버에 별도 API가 필요.
...
// -------- 관리자 카드 상세 --------
// ====== 관리자: 개별 카드 관리 (화이트 글래스 스타일) ======
app.get("/a/:token", (req, res) => {
  if (!isAdmin(req)) return res.redirect("/login?ref=" + encodeURIComponent(req.originalUrl));

  const token = req.params.token;
  ensureCard.run(token);
  const c = getCard.get(token);              // { token, label, balance, ... }
  const txs = listTx.all(token);             // 최근 50건
...
.balance .num{font-size:48px;font-weight:900;letter-spacing:.3px}
.badge{
  display:inline-block;padding:6px 12px;border-radius:999px;background:#eef2ff;border:1px solid #dbeafe;color:#1e3a8a;font-weight:700
}

.actions{display:flex;flex-wrap:wrap;gap:8px;margin:8px 0 12px}
.btn{padding:10px 14px;border:none;border-radius:10px;background:var(--accent);color:#fff;cursor:pointer;font-weight:700}
...
input,textarea{
  width:100%;padding:10px 12px;border:1px solid #e2e8f0;border-radius:12px;background:#ffffffd9
}
...
<div class="wrap">
  <div class="header">
    <div class="brand">개별 카드 관리</div>
    <div class="nav">
      <a href="/">대시보드</a>
      <a href="/admin/cards">카드 목록</a>
      <a href="/logout">로그아웃</a>
    </div>
  </div>
  ...
  <div class="grid">
    <!-- 좌: 조작 패널 -->
    <div class="card">
      <h2>${token} <span class="badge">${c.label || "라벨 없음"}</span></h2>
      <div class="sub">토큰(이름) · 라벨 · 현재 잔액을 관리합니다.</div>

      <div class="balance">
        <div class="num" id="bal">${c.balance}</div>
        <div class="sub">현재 잔액</div>
      </div>

      <div class="actions">
        <button class="btn" onclick="adj( 1)">+1</button>
        <button class="btn" onclick="adj( 5)">+5</button>
        <button class="btn" onclick="adj(10)">+10</button>
        <button class="btn gray"   onclick="adj(-1)">-1</button>
        <button class="btn gray"   onclick="adj(-5)">-5</button>
        <button class="btn gray"   onclick="adj(-10)">-10</button>
      </div>

      <div class="row">
        <input id="custom" type="number" placeholder="임의 증감 (예:+250 / -30)">
        <button class="btn" onclick="custom()">적용</button>
      </div>

      <div class="row">
        <input id="setv" type="number" placeholder="잔액을 특정 값으로 설정 (예: 280)">
        <button class="btn ghost" onclick="applySet()">잔액 설정</button>
      </div>

      <div class="row">
        <input id="label" type="text" value="${c.label||''}" placeholder="라벨 변경 (예: 2학년)">
        <button class="btn" onclick="saveLabel()">라벨 저장</button>
      </div>
    </div>
...
(function(){
  ...
  function filterAndSort(){
    ...
    let filtered = rows.filter(tr=>{
      const t = tr.getAttribute('data-token')||'';
      const l = tr.getAttribute('data-label')||'';
      return (!term || t.includes(term) || l.includes(term));
    });

    // 정렬
    if(currentSort==="name"){
      filtered.sort((a,b)=> (a.getAttribute('data-token')||"").localeCompare(b.getAttribute('data-token')||""));
    }else if(currentSort==="bal-desc"){
      filtered.sort((a,b)=> parseInt(b.getAttribute('data-balance')) - parseInt(a.getAttribute('data-balance')));
    }else if(currentSort==="bal-asc"){
      filtered.sort((a,b)=> parseInt(a.getAttribute('data-balance')) - parseInt(b.getAttribute('data-balance')));
    }else{
      // 기본순 = 원래 index (tr 순서 유지)
      filtered.sort((a,b)=> parseInt(a.querySelector('.idx').textContent) - parseInt(b.querySelector('.idx').textContent));
    }
...
// ====== 관리자: 카드(사람) 관리 — 최신버전 (화이트 글래스 + 검색/정렬 + 삭제) ======
...
label.small{display:block;font-size:12px;color:var(--muted);margin-bottom:6px}
.row{display:grid;grid-template-columns:1fr 1fr 1fr auto;gap:8px}
textarea,input[type="text"],input[type="number"]{width:100%;padding:10px 12px;border:1px solid #e2e8f0;border-radius:12px;background:#ffffffd9}
.btn{padding:10px 14px;border:none;border-radius:10px;background:var(--accent);color:#fff;cursor:pointer;font-weight:700}
...
tr:last-child td{border-bottom:none}
tbody tr:hover td{background:#fff}
th:nth-child(2), td:nth-child(2){text-align:left} /* 이름만 좌측 정렬 */
th:first-child,td:first-child{padding-left:12px}
th:last-child, td:last-child{padding-right:12px}
...
<div class="wrap">
  ...
  <div class="panel">
    <div class="grid">
      <div>
        <label class="small">단일 추가/수정</label>
        <form method="post" action="/admin/card/upsert" class="row">
          <input name="token" placeholder="이름/토큰 (예: hong01 또는 카드 UID)" required>
          <input name="label" placeholder="라벨(선택, 예: 홍길동)">
          <input type="number" name="balance" placeholder="초기 잔액(예: 300)" required>
          <button class="btn" type="submit">저장</button>
        </form>
      </div>
      <div>
        <label class="small">일괄 등록/수정 (한 줄에 한 명 — <code>토큰,초기잔액[,라벨]</code>)</label>
        <form method="post" action="/admin/card/bulk">
          <textarea name="bulk" rows="7" placeholder="hong01,300,홍길동\nkim02,280,김철수\nanna01,275,Anna"></textarea>
  ...
  <div class="panel">
    <div class="toolbar">
      <label class="input"><input id="q" placeholder="이름/라벨 검색"></label>
      <div class="segment" role="tablist" aria-label="정렬">
        <button class="active" data-sort="idx">기본순</button>
        <button data-sort="name">이름순</button>
        <button data-sort="bal-desc">잔액↓</button>
        <button data-sort="bal-asc">잔액↑</button>
    ...
    <div class="tableWrap">
      <table aria-label="카드 목록">
        ...
        <thead><tr><th>#</th><th>이름/토큰</th><th>라벨</th><th>잔액</th><th>열기</th></tr></thead>
        <tbody id="tb">${rowsHtml}</tbody>
      </table>
    </div>
  </div>
...
(function(){
  ...
  function filterAndSort(){
    ...
    // 정렬
    if(currentSort==="name"){
      filtered.sort((a,b)=> (a.getAttribute('data-token')||"").localeCompare(b.getAttribute('data-token')||"",'ko'));
    }else if(currentSort==="bal-desc"){
      filtered.sort((a,b)=> parseInt(b.getAttribute('data-balance')||'0') - parseInt(a.getAttribute('data-balance')||'0'));
    }else if(currentSort==="bal-asc"){
      filtered.sort((a,b)=> parseInt(a.getAttribute('data-balance')||'0') - parseInt(b.getAttribute('data-balance')||'0'));
    }else{
      filtered.sort((a,b)=> parseInt(a.querySelector('.idx').textContent) - parseInt(b.querySelector('.idx').textContent));
    }
